...
+ Cadastro das instituções de educação, abrangendo Universidades e Institutos Federais
...
# 3. Analisando o panorama Geral das Instituições de ensino Supeior
```{r}
tabela_frequencia <- dados_ies %>%
  ...
  select(-Total_Geral)
arrange(desc(`Percentual (%)`))   # <<< ordenação por maior percentual
```
...
# Cálculo e ordenação por população
...
ggplot(tabela_est, aes(x = fct_reorder(UF, `Val._absolutos`), y = `Val._absolutos`)) +
  ...
  labs(
    title = 'Figura 2: Distribuição de Instituições por UF',
    x = 'UF',
...
kable(
  ord_tipo,
  caption = "Tabela 5: Quantitativo e percentual de Cursos por tipo",
  format = "pipe"
)
...
Como podemos observar há muito mais ofertas e portanto, muito mais ingressantes em instituições privadas (88,52%) do que em instituições públicas, assim como concluientes. No entato, a taxa de conclusão, ou seja, a razão entre ingressantes e concluintes é muito maior em instituções públicas. Em outras palavras os alunos que ingressam em IES publicam concluem mais, o que indica que a evasão ou abandono em IES privadas é maior.
...
Na Figura 6, percebemos que, em geral, o nível de escolaridade das mulheres, em relação ao total delas, é maior que dos homens. Quanto maior a qualificação, maior é a "vantagem" das mulheres servidores em relação aos homens. Isso indica que as mulheres buscam maior qualificação nas funções técnicas das IES.
...
Apesar do número de instituições privadas serem superiores ao número de instituições públicas (2244 x 317), a distribuição dos técnicos é bastante equilibrada (Figura 7), sendo 50,89% em ies públcias e 49,11% em IES privadas. Isto pode indicar que funcionários em IES privadas podem exercer diversas funções ou também retratar menos serviços oferecidos ou até mesmo reutilização de maão de obra.
...
Vamos analisar agora a faixa etária dos docentes das IES, lembrando que não temos esses dados em relação aos técnicos. A tabela 11 abaixo exibe a faixa etária dos docentes da IES.
...
faixa_etaria <- docentes %>%
  select(QT_DOC_EX_0_29, QT_DOC_EX_30_34, QT_DOC_EX_35_39, QT_DOC_EX_40_44,
  ...
  mutate(
    faixa_etaria = case_when(
      ...
      faixa_etaria_original == "QT_DOC_EX_35_39"  ~ "35 a 39 anos",
  ...
  summarise(total_docentes = sum(quantidade, na.rm = TRUE), .groups = "drop") %>%
  mutate(
    faixa_etaria = factor(faixa_etaria,
      levels = c("Até 29 anos", "30 a 34 anos", "35 a 39 anos",
  ...
  mutate(
    Total = format(total_docentes, big.mark = ".", decimal.mark = ","),
    `% do total` = paste0(percentual, "%")
  ) %>%
  select(Faixa = faixa_etaria, Total, `% do total`)

# Tabela final
faixa_etaria %>%
  kable(
    format = "html",
    caption = "<div style='text-align:center; font-weight:bold; font-size:16px;'>Tabela 11: Distribuição etária dos docentes (total e percentual)</div>",
    align = c("l", "r", "r")
  ) %>%
...
Também exibimos a faixa etária dos docentes das ies no gráfico da Figura 11. Podemos perceber que, em sua maioria, possui entre 40 e 44 anos (representando 18,2% do total). A faixa de 45-49 anos também apresenta um número elevado de docentes, com 16,3%. 
...
nm <- names(faixa_etaria)

total_col_candidates <- grep("^total_docentes$|^total$|Total_Geral$|^Total$", nm, ignore.case = TRUE, value = TRUE)
percent_col_candidates <- grep("^percentual$|percent|% do total|perc", nm, ignore.case = TRUE, value = TRUE)
faixa_col_candidates <- grep("^faixa_etaria$|^Faixa$|^faixa$|^faixa_etaria_original$", nm, ignore.case = TRUE, value = TRUE)

total_col <- if (length(total_col_candidates) > 0) total_col_candidates[1] else NA_character_
percent_col <- if (length(percent_col_candidates) > 0) percent_col_candidates[1] else NA_character_
faixa_col <- if (length(faixa_col_candidates) > 0) faixa_col_candidates[1] else NA_character_

if (is.na(total_col) || is.na(percent_col) || is.na(faixa_col)) {
  stop("Não localizei automaticamente as colunas necessárias. Rode names(faixa_etaria) e verifique:\n",
       "Esperado: coluna de faixa (ex.: 'Faixa' ou 'faixa_etaria'), coluna de total (ex.: 'total_docentes' ou 'Total'), coluna de percentual (ex.: 'percentual' ou '% do total').")
}

raw_total <- faixa_etaria[[total_col]]
total_vec <- if (is.numeric(raw_total)) {
  as.numeric(raw_total)
} else {
  as.numeric(gsub("\\.", "", as.character(raw_total)))
}

# percentual: limpar '%' e vírgula decimal
raw_perc <- faixa_etaria[[percent_col]]
perc_vec <- as.character(raw_perc)
perc_vec <- gsub("\\s+", "", perc_vec)
perc_vec <- gsub("%", "", perc_vec, fixed = TRUE)
perc_vec <- gsub(",", ".", perc_vec, fixed = TRUE)
perc_vec <- as.numeric(perc_vec)

# faixa (rótulo)
faixa_vec <- as.character(faixa_etaria[[faixa_col]])


plot_df <- tibble(
  faixa_etaria = faixa_vec,
  total_docentes = total_vec,
  percentual = perc_vec
)

# 4) garantir ordem lógica das faixas
plot_df <- plot_df %>%
  mutate(
    faixa_etaria = factor(faixa_etaria,
                          levels = c("Até 29 anos", "30 a 34 anos", "35 a 39 anos",
...
if (any(is.na(plot_df$total_docentes))) stop("Há NA em total_docentes após conversão. Verifique as colunas.")
if (any(is.na(plot_df$percentual))) stop("Há NA em percentual após conversão. Verifique as colunas.")

# 5) gráfico
ggplot(plot_df, aes(x = faixa_etaria, y = total_docentes, fill = faixa_etaria)) +
  geom_col(alpha = 0.8, width = 0.7, show.legend = FALSE) +
  geom_text(aes(label = paste0(comma(total_docentes), "\n(", percentual, "%)")),
            vjust = -0.3, size = 4, fontface = "bold", lineheight = 0.8) +
  labs(
    title = "Figura 11: Distribuição de docentes por faixa etária",
    x = "Faixa Etária",
    y = "Total de Docentes"
  ) +
...
```
Vale salientar que a quarta faixa etária que possui mais docentes é a dos docentes que tem mais de 60 anos (cerca de 13,5%), o que acende um alerta, já que estes estão próximos à aposentadoria. A faixa etária com menor número de docentes é que corresponde aos docentes com menos de 29 anos, representando apenas 3,5% do total. Isso indica uma menor presença de docentes iniciantes na carreira do magistério nas IES e  uma entrada mais tardia na carreira docente ou menor atratividade para os mais jovens.. 


### 3.6.4. Perfil étinico dos docentes

Em relação à etnia dos docentes das IES, 61,9% se declaram brancos e apenas 3,2% se declaram pretos. Neste meio, 19,9% se declaram pardos. Um número significativo não declara a cor (13,5%), conforme evidencia a Tabela 12 e a Figura 12 abaixo.
...
#Preparando dados
etnia <- docentes %>%
  select(QT_DOC_EX_BRANCA, QT_DOC_EX_PRETA, QT_DOC_EX_PARDA,
         QT_DOC_EX_AMARELA, QT_DOC_EX_INDIGENA, QT_DOC_EX_COR_ND) %>%
  pivot_longer(
    cols = everything(),
    names_to = "Etnia_original",
    values_to = "Quantidade"
  ) %>%
  mutate(
    Etnia = case_when(
      Etnia_original == "QT_DOC_EX_BRANCA"   ~ "Branca",
      Etnia_original == "QT_DOC_EX_PRETA"    ~ "Preta",
      Etnia_original == "QT_DOC_EX_PARDA"    ~ "Parda",
      Etnia_original == "QT_DOC_EX_AMARELA"  ~ "Amarela",
      Etnia_original == "QT_DOC_EX_INDIGENA" ~ "Indígena",
      Etnia_original == "QT_DOC_EX_COR_ND"   ~ "Não Declarada",
      TRUE ~ Etnia_original
    )
  ) %>%
  group_by(Etnia) %>%
  summarise(
    total_docentes = sum(Quantidade, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    Etnia = factor(Etnia, levels = c("Branca", "Preta", "Parda", "Amarela", "Indígena", "Não Declarada")),
    percentual = round(total_docentes / sum(total_docentes, na.rm = TRUE) * 100, 1)
  ) %>%
  arrange(Etnia)
...
etnia_display <- etnia %>%
  mutate(
    Total_fmt = comma(total_docentes),
    Perc_fmt  = paste0(percentual, "%")
  ) %>%
  select(Etnia, Total_fmt, Perc_fmt) %>%
  rename(
    `Etnia` = Etnia,
    `Total de Docentes` = Total_fmt,
    `Percentual` = Perc_fmt
  )
...
etnia_display %>%
  kable(
    format = "html",
    caption = "<div style='text-align: center; font-size: 16px; font-weight: bold;'>Tabela 12: DISTRIBUIÇÃO DE DOCENTES POR ETNIA</div>",
    align = c("l", "r", "r"),
    escape = FALSE
  ) %>%
  kable_styling(
...
ggplot(etnia, aes(x = Etnia, y = total_docentes)) +
  ...
  geom_text(aes(label = paste0(scales::comma(total_docentes), "\n(", percentual, "%)")), 
            vjust = -0.3, size = 4, fontface = "bold", lineheight = 0.8) +
  labs(
    title = "Figura 12: Distribuição de docentes por Etnia",
    x = "Etnia",
    y = "Total de Docentes"
  ) +
  scale_y_continuous(
...
### 3.7.2 Faixa Etária
Realizamos a análise a seguir, na Figura 14, com o intuito de mostrar a distribuição dos alunos por faixa etária. 
...
ggplot(faixa_etaria, aes(x = Faixa_Etaria, y = Quantidade, fill = Faixa_Etaria)) +
  geom_col() +
  labs(
    title = "Figura 14: Distribuição de Alunos por Faixa Etária",
    x = "Faixa Etária",
    y = "Quantidade de Alunos"
  ) +
  scale_y_continuous(labels = label_comma(big.mark = ".", decimal.mark = ",")) +
...
ggplot(etnia, aes(x = Etnia, y = Quantidade, fill = Etnia)) +
  geom_col() +
  labs(
    title = "Figura 1: Distribuição de Alunos por Etnia",
    x = "Etnia",
    y = "Quantidade de Alunos"
  ) +
  scale_y_continuous(labels = label_comma(big.mark = ".", decimal.mark = ",")) +
...
A análise dos microdados da Educação Superior no Brasil revelou um total de 2.561 Instituições de Ensino Superior (IES). A distribuição regional mostra concentração no Sudeste (42,01%), seguido pelo Nordeste (23,58%), Sul (15,19%), Centro-Oeste (11,56%) e Norte (7,65%). Em termos proporcionais à população, o Centro-Oeste apresenta maior densidade de IES, enquanto o Sudeste, apesar de liderar em números absolutos, possui menor proporção relativa.

No recorte por estados, São Paulo e Minas Gerais concentram os maiores quantitativos, enquanto Roraima, Amapá e Acre figuram entre os menores. Observou-se também que, embora estados populosos como Rio de Janeiro tenham grande demanda, apresentam menos IES do que estados como Paraná e Bahia, evidenciando disparidades regionais.

Quanto à natureza das instituições, verificou-se a predominância das privadas (87,6%), em contraste com apenas 12,4% de públicas. Apesar disso, as IES públicas apresentaram taxas de conclusão superiores, indicando maior permanência e menor evasão.

No que se refere aos cursos, identificou-se forte concentração em áreas empresariais e tecnológicas, com destaque para Pedagogia, Sistemas de Informação e Administração entre os mais ofertados. Os cursos tecnólogos representam quase metade da oferta (47%), refletindo a busca por formações rápidas e voltadas ao mercado de trabalho. Em contrapartida, apenas 18,95% dos cursos são licenciaturas, o que pode estar relacionado à desvalorização da carreira docente.

A análise das vagas e matrículas mostrou que o Sudeste lidera em oferta e preenchimento, enquanto o Centro-Oeste apresenta menor volume. Contudo, em termos de taxa de conclusão, as instituições públicas se destacam, reforçando sua papel na formação acadêmica.

No perfil dos funcionários técnicos, verificou-se predominância feminina (55,4%) e maior concentração nos níveis médio, superior e especialização. Já entre os docentes, identificou-se um total de 374.501 profissionais, com leve predominância masculina (52,2%). A maioria possui titulação de doutorado (53%) ou mestrado (32%), evidenciando elevada qualificação. Entretanto, persistem desigualdades étnico-raciais: 61,9% dos docentes se declaram brancos, enquanto pretos e indígenas representam menos de 4% do total.

Por fim, no perfil discente, observou-se uma disparidade significativa entre matrículas e conclusões, indicando altas taxas de evasão. A maioria dos ingressantes está na faixa etária de 18 a 24 anos, mas há também presença expressiva de adultos jovens. Em termos étnicos, predominam estudantes brancos, seguidos por pardos, enquanto pretos, indígenas e amarelos apresentam baixa representatividade.

## 4.2. Discussão

Os resultados revelam um sistema de ensino superior marcado por contrastes: expansão e diversidade regional, mas também concentração em poucos estados; predominância de instituições privadas, mas maior taxa de conclusão nas públicas; elevada qualificação docente, mas persistência de desigualdades étnico-raciais; ampla oferta de cursos tecnólogos, mas baixa valorização das licenciaturas.

## 4.3. Considerações Finais

Em síntese, o estudo evidencia desafios estruturais — como evasão, desigualdade étnico-racial e desvalorização da docência —, mas também avanços significativos, como a expansão das IES em diferentes regiões, a qualificação crescente do corpo docente e a diversificação da oferta de cursos. Tais achados reforçam a necessidade de políticas públicas voltadas à equidade regional, à valorização da carreira docente e ao incentivo à permanência estudantil, assegurando que o ensino superior cumpra seu papel de democratização do conhecimento e promoção do desenvolvimento social.