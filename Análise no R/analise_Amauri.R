...
dadosA_frequentes <- dadosA %>%
  group_by(Curso) %>%
summarise(
  Quantidade_de_dadosA = n(),.groups = 'drop') %>%
  mutate(Total_Geral = sum(Quantidade_de_dadosA),`Percentual (%)` = round((Quantidade_de_dadosA / Total_Geral) * 100, 2)
  ) %>%
  select(-Total_Geral)

ord_dadosA <- dadosA_frequentes %>%
  arrange(desc(Quantidade_de_dadosA))%>%
slice_head(n = 10)
print(ord_dadosA)
...
#Vaga e ingressantes por tipo de curso
vagasA<- dadosA %>%
  group_by(Tipo_de_curso) %>%
  summarise(
...
#Montagem banco de dados analise 3.4
dadosB <-data.frame(
  "regiao"= case_when(
...
#Vagas e ingressantes por regiao
vagas_regiao<- dadosB %>%
  group_by(regiao) %>%
  summarise(
...
#Concluintes por região e tipo de instituição
